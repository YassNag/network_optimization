...
function resolution_modele(instance)
    ...
    choix = choix_binaire("\n --> Souhaitez-vous ajouter les contraintes de renforcement R2 (pas de deploiement de HAPS --> Pas de placement de relais): (o/n) ? ")
    ajout_R2 = choix == "o" ? true : false
    choix = choix_binaire("\n --> Souhaitez-vous ajouter les contraintes de dominance de positions (o/n) ? ")
    ajout_R3 = choix == "o" ? true : false
    choix = choix_binaire("\n --> Souhaitez-vous ajouter les contraintes de couverture-connexité? ")
    ajout_R4 = choix == "o" ? true : false
    choix = choix_binaire("\n --> Souhaitez-vous ajouter un callback vide ? ")
    global callback_vide = choix == "o" ? true : false
    if callback_vide == false
        choix = choix_binaire("\n --> Souhaitez-vous ajouter les contraintes de couverture (cover inequalities) ? ")
        ajout_R5 = choix == "o" ? true : false
        if ajout_R5==true
            choix = choix_binaire("\n --> Souhaitez-vous ajouter un ajout des inégalités de couverture uniquement à la racine ? ")
            global only_on_root = choix == "o" ? true : false
            if  only_on_root==false
                println("\n --> Avec quelle fréquence souhaitez-vous que le callback soit appellé? ")
                global frequency = parse(Int64, readline(stdin))
            end
            choix = choix_binaire("\n --> Souhaitez-vous séparer les contraintes de couverture avec la méthode exacte ? ")
            global separation_exacte = choix == "o" ? true : false
    ...
    relaxed=false
    temp_creation_modele = @elapsed modele = creation_modele(instance, E1, E2, E4, ajout_R1, ajout_R2, ajout_R3, ajout_R4, ajout_R5, relaxed, nb_const)
    ...
    relaxed=true
    relaxed_modele = creation_modele(instance, E1, E2, E4, ajout_R1, ajout_R2, ajout_R3, ajout_R4,ajout_R5, relaxed, nb_const)
    z2= relaxation_value(relaxed_modele, instance)
    relaxed=false
    ...
    if statut == 1 # Solution disponible
        ...
        solution = lecture_resultats_modele(instance, E1, E2, modele, Vertices, Arcs, Ebis, relaxed)
...
function creation_modele(instance, E1, E2, E4, ajout_R1, ajout_R2, ajout_R3, ajout_R4, ajout_R5, relaxed, nb_const)
...
if ajout_R4 == true
    ...
    if ajout_R5
        lowerP=[Int[] for h in 1:nH]
        lowerW=[Int[] for h in 1:nH]
        for h1 in 1:nH
            for h2 in 1:nH
                if P[h2]<=P[h1]
                    push!(lowerP[h1],h2)
